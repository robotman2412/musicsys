...
public:
		...
		std::size_t bytes_transferred)
	{
		...
		// Send the response
		handle_request(docRoot, std::move(req_), lambda_);
	}
...
static std::shared_ptr<HttpListener> listener;
...
void startHttpServer(int port, std::string doc_root, int threads) {
	...
	listener = std::make_shared<HttpListener>(
		*ioc,
		tcp::endpoint{address, port}
	);
	listener->run();
	
	// Start the HTTP server threads.
